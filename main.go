...
import (
	...
	"github.com/fullstacksam23/gofer/tools"
			...
			for _, tc := range msg.ToolCalls {
				if tc.Type == "function" {
					if tc.Function.Name == "Read" {
						...
						fileContent, err := tools.ReadFileTool(args.FilePath)
						...
						writeErr := tools.WriteFileTool(args.FilePath, args.Content)
						if writeErr != nil {
							log.Fatal(writeErr)
						}
						...
						bashOutput, bashErr := tools.BashTool(args.Command)
						if bashErr != nil {
							log.Fatal(bashErr)
						}
						conversation = append(conversation, openai.ChatCompletionMessageParamUnion{
